...
'''--------------------시리얼 관련 설정-----------'''
import serial #pyserial 패키지를 설치해야합니다.

ser = serial.Serial('/dev/tty.usbmodem31101', 9600) # mac의 경우 터미널에서 ls /dev/tty.*으로 포트 검색
'''--------------------시리얼 관련 설정-----------'''
...
os.environ['GOOGLE_API_KEY'] = 'YOUR API KEY' # 자신의 키로 교체
                                        ...
                                        for function_call in function_calls:
                                            ...
                                            if name == "set_light_values":
                                                ...
                                                try:
                                                    ...
                                                    print(f"Set fan values result: {result}")

                                                    # 시리얼로 팬 속도 전송
                                                    '''--------------------시리얼 관련 설정-----------'''

                                                    speed = result["speed"]
                                                    serial_command = f"FAN{speed}\n".encode('utf-8')
                                                    ser.write(serial_command)
                                                    print(f"Sent to serial: FAN{speed}")
